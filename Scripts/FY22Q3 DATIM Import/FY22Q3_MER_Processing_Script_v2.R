...
#join NDOH to MFL
ndoh_join2 <- df_fac %>% 
  left_join(ndoh_all,  by = c("old_ou5code" = "Code")) 
...
# write_csv(missing_disagg2, "Dataout/Q3_datimuid_unmapped.csv")

#address PrEP_CT TEST RESULT AND AGE/SEX DISAGG -------------------------------

#first, map to the age groups

ndoh_prep <- ndoh_clean2 %>% 
...
ndoh_prep_age <- ndoh_prep %>%
  left_join(df_map_distinct %>% select(-c(Result)), by = c("Test Result/Outcome/Duration" = "Test Resuts/Outcome/Duration",
...
ndoh_prep_test <- ndoh_prep_test %>%
  left_join(df_map_distinct %>%
              select(-c(Sex, CoarseAgeGroup)) %>% 
              filter(str_detect(dataElement, "TestResult")),
...
ndoh_prep_final <- bind_rows(ndoh_prep_age, ndoh_prep_test)
...
ndoh_rtt_age_sex <- ndoh_rtt_age_sex %>%
  left_join(df_map_distinct %>%
              select(-c(`Test Resuts/Outcome/Duration`)) %>% 
              filter(str_detect(dataElement, "Age/Sex/HIVStatus")), by = c("Sex", "CoarseAgeGroup", "Result","indicator",
...
ndoh_rtt_nocontact <- ndoh_clean2 %>% 
  filter(indicator == "TX_RTT") %>%
  group_by(usaid_facility, ou5uid, datim_uid, old_ou5code, period, DSD_TA,
           Province, District, SubDistrict, Facility,
...
ndoh_rtt_nocontact <- ndoh_rtt_nocontact %>%
  left_join(df_map_distinct %>%
              select(-c(Sex, CoarseAgeGroup, Result)) %>% 
              filter(str_detect(dataElement, "ARTNoContactReasonIIT")),
            by = c("Test Result/Outcome/Duration" = "Test Resuts/Outcome/Duration",
...
ndoh_rtt_all <- bind_rows(ndoh_rtt_age_sex, ndoh_rtt_nocontact)

# ndoh_rtt_age_sex %>% filter(is.na(dataElement)) %>% 
# select(indicator, DSD_TA, District, Facility, Sex, CoarseAgeGroup, `Test Result/Outcome/Duration`,
#        Result, numeratordenom, dataElement)
...
#add PrEP and TX RTT to final import
import_file_clean <- bind_rows(import_file_clean, ndoh_prep_final, ndoh_rtt_all)

write_csv(import_file_clean, "Dataout/FY22Q3-import-file-readyfor-bmw-qc.csv")
...
kp_indic <- c("PrEP_New_KP", "PrEP_CT_KP",
              #"HTS_TST_KP",
...
#join NDOH to MFL
ndoh_join_kp <- df_fac %>% 
  left_join(ndoh_kp_agg,  by = c("old_ou5code" = "Code"))
...
ndoh_kp_prep_age <- ndoh_kp_prep_age %>% 
  select(mech_code, mech_uid, Facility, datim_uid, dataElement, dataElement_uid,categoryOptionComboName, categoryOptionCombo_uid,
         Total, SubDistrict, District) %>% 
  rename(value = Total,
         orgUnit_uid = datim_uid,
...
#address PrEP_CT - now, map test result
ndoh_kp_prep_test <- ndoh_kp_clean %>% 
  filter(indicator == "PrEP_CT") %>%
  group_by(usaid_facility, ou5uid, datim_uid, old_ou5code, period, DSD_TA,
           Province, District, SubDistrict, Facility, `Test Result/Outcome/Duration`,
           Result, indicator, numeratordenom) %>% 
...
ndoh_kp_prep_test <- ndoh_kp_prep_test %>%
  left_join(df_map_distinct %>%
              select(-c(Sex, CoarseAgeGroup)) %>% 
              filter(str_detect(dataElement, "TestResult")),
            by = c("Test Result/Outcome/Duration" = "Test Resuts/Outcome/Duration",
...
ndoh_kp_prep_final <- bind_rows(ndoh_kp_prep_age, ndoh_kp_prep_test) 
  
  
    janitor::get_dupes(ndoh_kp_prep_final) %>% view()
...
import_file_clean_kp <- bind_rows(import_file_clean_kp, ndoh_kp_prep_final)
...
final_import_file_Q3 <-  bind_rows(import_file_clean, import_file_clean_kp)
...
write_csv(final_validation_Q3, "Dataout/Partner Import Files/FY22Q3_ALL_Import-not-dedupped.csv")


duplicates <- janitor::get_dupes(final_import_file_Q3) %>% 

#BROADREACH
FY22Q3_Broadreach_import <- final_import_file_Q3 %>% 
  filter(!is.na(dataElement)) %>% 
  filter(mech_code == 70287)

#RTC
FY22Q3_RTC_import <- final_import_file_Q3 %>% 
  filter(!is.na(dataElement)) %>% 
  filter(mech_code == 70290)

#WRHI
FY22Q3_WRHI_import <- final_import_file_Q3 %>% 
  filter(!is.na(dataElement)) %>% 
  filter(mech_code == 70301)

#ANOVA
FY22Q3_ANOVA_import <- final_import_file_Q3 %>% 
  filter(!is.na(dataElement)) %>% 
  filter(mech_code == 70310)

#MATCH
FY22Q3_MATCH_import <- final_import_file_Q3 %>%
  filter(!is.na(dataElement)) %>% 
